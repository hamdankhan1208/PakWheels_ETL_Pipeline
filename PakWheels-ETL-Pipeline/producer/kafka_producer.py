    ...
    for listing in listings:
        try:
            ...
            location = listing.find_element(
                By.CSS_SELECTOR, "ul.search-vehicle-info li"
            ).text
            specs = listing.find_elements(
                By.CSS_SELECTOR, "ul.search-vehicle-info-2 li"
            )
            year = specs[0].text if len(specs) > 0 else ""
            mileage = specs[1].text if len(specs) > 1 else ""
            engine = specs[3].text if len(specs) > 3 else ""
            url = listing.find_element(By.CSS_SELECTOR, "a.car-name").get_attribute(
                "href"
            )
            img = listing.find_element(
                By.CSS_SELECTOR, "li.total-pictures-bar-outer img"
            ).get_attribute("src")
